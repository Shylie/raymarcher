...
__device__ Vec Trace(Vec origin, Vec direction, int bounceCount, xorwow& random)
{
	Vec sampledPosition, normal, color, attenuation = 1;

	for (int bc = 0; bc < bounceCount; bc++)
	{
...
__global__ void Render(Vec* cols, int w, int h, int s, int bc, Vec position, Vec goal, Vec left, Vec up, int sx, int sy, int ex, int ey)
{
	for (int i = sx + threadIdx.x + blockIdx.x * blockDim.x; i < ex && i < w; i += blockDim.x * gridDim.x)
	{
		for (int j = sy + threadIdx.y + blockIdx.y * blockDim.y; j < ey && j < h; j += blockDim.y * gridDim.y)
		{
			...
			{
				col += Trace(position, !(goal + left * (i - w / 2.0f + random()) + up * (j - h / 2.0f + random())), bc, random);
			}
			cols[i + j * w] = col / s;
		}
	}
